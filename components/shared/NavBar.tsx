     ...
     className={`
      ...
      >
        <div className={`mx-auto bg-[#004d40]  text-white   font-serif transition-colors w-full  flex  2xl:max-w-5xl lg:max-w-4xl py-2 lg:px-8  px-10 2xl:px-12  lg:rounded-3xl   items-center justify-between`}>
          ...
          {/* Desktop Menu */}
          <div className={`hidden lg:items-center text-lg xl:text-2xl font-medium  font-serif    lg:justify-center space-x-2 lg:space-x-9 2xl:space-x-11 w-full lg:flex`}>
        ...
        <AnimatePresence>
          {isMenuOpen && (
            <motion.div
              ...
              className="fixed top-0 right-0 h-dvh w-full bg-[#fbfaf6] shadow-lg flex flex-col justify-between py-12 px-6 z-50"
            >
              ...
              <nav className="flex flex-col h-full w-full mt-10 font-normal text-3xl gap-y-6">
                {['Home', 'About', 'Services', 'Gallery', 'Contact'].map((item, index) => (
                  ...
                  >
                    <Link
                      ...
                      className={`hover:text-[#D4AF37] transition-colors ${item === 'Home' ? 'text-bronze' : ''
                        }`}
                      onClick={toggleMenu}
                    >
                      {item}
        ...
        >
          Get Quote
                  ...
                  <Button size={"sm"} className="text-[#013220] border    hover:bg-Gold border-yellow-500 bg-Gold hover:text-white rounded-lg font-medium">
                    <LeafIcon className="w-4 h-4 mr-2 " />
                    GET YOUR QUOTE
                  </Button>
                </motion.div>
